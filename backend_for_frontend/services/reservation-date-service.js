...
const all = async (req, res) => {
    const response = await fetch(`http://localhost:8000/reservationDate/all`)
...
const addReservation = async (req, res) => {
    ...
    if(dateOfVisit == '' || userId == '' || nameOfVisitor == '' || reserverId == ''){
        ...
        const data = {
          ...
          await fetch('http://localhost:8000/reservationDate/add', {
            method: 'POST', 
            headers: {
...
const updateReservation = async (req, res) => {
    ...
    if(!req.params) {
        ...
        const response = await fetch(`http://localhost:8000/reservationDate/update/${visitId}/${dateOfVisit}`, {
            method: 'PUT', 
        });
